...
if (isset($_REQUEST['emp']) && $_REQUEST['act'] == 'app') {
    ...
    mysql_query("UPDATE table_payslip SET Status = 2 WHERE Emp_no = '$emp' AND Adj_periodcovered = '$date'");
    header("Location: VerifiedData.php");
}
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="card">
                            ...
                            <div style="background-color: #fff; min-height: 80vh;" class="body">
                                ...
                                <?php 
                                    $datee = $_SESSION['INTRA_DATE'];
                                    $date = explode(' ', $datee);
                                    $month = date('F', mktime(null, null, null, $date[1], 1));
                                    echo '<p style="font-weight: bold; font-size: 18px; text-align: center;"> Payroll ' . $month . ' ' . $date[0] . '</p>';
                                ?><br>
                                ...
                                <table id="example" class="table-bordered stripe row-border order-column" cellspacing="0" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th rowspan="2" class="fixed-side">Action</th>
                                        </tr>
                                        <tr>
                                            <!--GSIS-->
                                            <th>RLIP</th>
                                    ...
                                    <tbody>
                                        <?php
                                        $datee = explode(' ', $datee);
                                        $query = mysql_query("SELECT * FROM table_payslip WHERE year_covered = '$datee[0]' AND month_covered = '$datee[1]' AND verified = 'Yes'");
...
//                    . "     ON C.division_id = D.division_id");,
                                        while ($row = mysql_fetch_array($query)) {
                                            ...
                                            if ($row['verified'] == 'Yes') {
                                                ...
                                                echo '<a class="confirmme btn bg-green btn-xs waves-effect" data-toggle="tooltip" data-placement="top" title="Confirm" name="confirm" data-emp="' . $row['Emp_no'] . '" data-date="' . $row['Adj_periodcovered'] . '" target="_blank"><i class="material-icons">done</i></a>&nbsp;';
        ...
        $(document).ready(function () {
            ...
            $('#example').on('click', 'a.confirmme', function (e) {
                ...
                function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'VerifiedData.php?emp=' + emp + '&date=' + date + '&act=app',
                        success: function () {
                            $('#example').attr("id");
