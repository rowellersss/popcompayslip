...
//            $query2 = mysql_query("Select * from table_empinfo where Employee_Id = '$empid'");
//            $row = mysql_fetch_assoc($query2);
//            $name = ucfirst(mysql_real_escape_string($row['Firstname'])) . " " . ucfirst(mysql_real_escape_string($row['Lastname']));
            $d2 = mysql_real_escape_string($_POST['reqby']);
            $d4 = mysql_real_escape_string($_POST['desc']);
            $sel = mysql_query("Select * from table_empinfo where Employee_Id = '$d2'");

            $row6 = mysql_fetch_assoc($sel);
            $pos = $row6['Position'];
            $fullname = $row6['Firstname'] . " " . $row6['Lastname'];

            $query3 = mysql_query("Select * from table_position where position_id = '$pos'");
            $row2 = mysql_fetch_assoc($query3);

            $divid = $row2['division_id'];
            $query4 = mysql_query("Select * from table_division where division_id = '$divid'");
            $row3 = mysql_fetch_assoc($query4);

            $div = $row3['division_description'];
            ...
            $reqby = $dur = $desc = "";
            $reqbyErr = $durErr = $descErr = "";
            if ($_SERVER["REQUEST_METHOD"] == "POST") {

                if (empty($_POST["reqby"])) {
                    $reqbyErr = "Requested by is required!";
                } else {
                    $reqby = $_POST["reqby"];
                }
                ...
                if (empty($descErr) && empty($durErr) && empty($reqbyErr)) {
                    $query = mysql_query("insert into table_servicereq (Emp_Id, DateofReq, TimeOfReq, ReqBy, DivUnitRPO, DescOfReq, Status) value('$d2','$date','$time','$fullname','$div','$d4','Approved')");
                    echo '<script language="javascript">';
                    echo 'alert("Successful\n\n")';
                    echo '</script>';
                } elseif (empty($descErr) && empty($durErr) && empty($reqbyErr)) {
                    echo '<script language="javascript">';
                    echo 'alert("' . $reqbyErr . ' ' . $dur . ' ' . $descErr . '")';
                    echo '</script>';
                }
            }
        }
        if (accelvl4() == 0) {
            ...
            $query2 = mysql_query("Select * from table_empinfo where Employee_Id = '$empid'");
            $row = mysql_fetch_assoc($query2);
            $reqby = ucfirst(mysql_real_escape_string($row['Firstname'])) . " " . ucfirst(mysql_real_escape_string($row['Lastname']));

            $dur = mysql_real_escape_string($row['Position']);
            $query3 = mysql_query("Select * from table_position where position_id = '$dur'");
            $row2 = mysql_fetch_assoc($query3);
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if (empty($_POST["desc"])) {
                    $desc1Err = "Description is required!";
                } else {
                    $desc1 = $_POST["desc"];
                }
                if (empty($desc1Err)) {
                    if (accelvl5() != 0) {
                        $query = mysql_query("insert into table_servicereq (Emp_Id, DateofReq, TimeOfReq, ReqBy, DivUnitRPO, DescOfReq, DateApproved, ApprovedBy, Status) value('$empid','$date','$time','$reqby','$div','$desc','$date','$reqby','Approved')");
                    } else {
                        $query = mysql_query("insert into table_servicereq (Emp_Id, DateofReq, TimeOfReq, ReqBy, DivUnitRPO, DescOfReq, DateApproved) value('$empid','$date','$time','$reqby','$div','$desc','$date')");
                    }
                    echo '<script language="javascript">';
                    echo 'alert("Successful\n\n")';
                    echo '</script>';
                } elseif (!empty($desc1Err)) {
                    echo '<script language="javascript">';
                    echo 'alert("' . $desc1Err . '")';
                    echo '</script>';
                }
                        ...
                        <div class="card">
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" class="body">
                                <div style="min-height: 77vh;right: 0; width: auto; padding: 40px;">
                                    <form method="post" action="ServiceRequest.php">
                                        <?php
                                        if (isset($_SESSION['INTRA_EMP_ID'])) {
                                            if (accelvl4() != 0) {
                                                ?>
                                                <h2 class="card-inside-title">Requested by</h2>
                                                <div class="row clearfix">
                                                    <div class="col-sm-6">
                                                        <div class="form-group">
                                                            <div class="form-line">
        <!--                                                                <input name="reqby" type="text" class="form-control"/>-->
                                                                <select name="reqby" class="form-control show-tick" data-live-search="true">
                                                                    <option value="">-- Please select --</option>
                                                                    <?php
                                                                    $sel = mysql_query("Select * from table_empinfo where Employee_Id BETWEEN 1 AND 90 ORDER BY Lastname asc");
                                                                    while ($row1 = mysql_fetch_array($sel)) {
                                                                        echo '<option value="' . $row1['Employee_Id'] . '">' . $row1['Lastname'] . ' ' . $row1['Firstname'] . '</option>';
                                                                    }
                                                                    ?>
                                                                </select>
                                                            </div>
                                                ...
                                                <h2 class="card-inside-title">Description of Request: <i>(Please clearly write down the details of the request)</i></h2>
                                                <div class="row clearfix">
                                                    <div class="col-sm-6">
                                                        <div class="form-group">
                                                            <div class="form-line">
                                                                <textarea name="desc" cols="50" rows="10" class="form-control no-resize" placeholder="Please enter the description of your request..."></textarea>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>

                                                <input type="submit" name="submit" class="btn bg-indigo waves-effect" value="Submit">
                                                <input type="submit" name="back" class="btn bg-light-blue waves-effect" value="Back" >
                                                <?php
                                            }
                                        }
                                        ?>
