...
if (!empty($_POST['year'])) {
    $date = $d1 . '-' . $d2 . '-01';
    $sql = mysql_query("SELECT * FROM table_specialpayroll WHERE Emp_no = '$empid' AND year_covered = '$d1' AND month_covered = '$d2'");
    $numcount = mysql_num_rows($sql);
    print_r($numcount);
} elseif (!empty($_POST['year2'])) {
    $date = $d3 . '-01-01';
    $date2 = $d3 . '-12-31';
    $sql = mysql_query("SELECT * FROM table_specialpayroll WHERE Emp_no = '$empid' AND year_covered = '$d3'");
    $numcount = mysql_num_rows($sql);
} elseif (!empty($_POST['year3'])) {
    $date = $d5 . '-01-01';
    $date2 = $d5 . '-' . $d6 . '-31';   
    $sql = mysql_query("SELECT * FROM table_specialpayroll WHERE Emp_no = '$empid' AND year_covered = '$d5' AND month_covered BETWEEN '01' AND '$d6'");
    $numcount = mysql_num_rows($sql);
} elseif (!empty($_POST['year4'])) {
    $date = $d7 . '-' . $d8 . '-01';
    $date2 = $d7 . '-' . $d9 . '-01';
    $sql = mysql_query("SELECT * FROM table_specialpayroll WHERE Emp_no = '$empid' AND year_covered = '$d7' AND month_covered BETWEEN '$d8' AND '$d9'");
    $numcount = mysql_num_rows($sql);
}
