...
if (isset($_SESSION['INTRA_EMP_ID'])) {
    $accelvl0 = accelvl0();
    if ($accelvl0['position'] != 12 AND $accelvl0['position'] != 23 AND $accelvl0['position'] != 95) {
        header("Location: index.php");
    }
}
...
if (isset($_REQUEST['emp']) && $_REQUEST['act'] == 'app') {
    $emp = $_REQUEST['emp'];
    $date = $_REQUEST['date'];
    $date_ben = explode('-',$_REQUEST['date_ben']);
    $date_ben = $date_ben[0].'-'.$date_ben[1];
    $key = $_REQUEST['key'];


        mysql_query("UPDATE table_specialpayroll SET verified = 'Yes' WHERE Emp_no = '$emp' AND In_date = '$date'");
        mysql_query("UPDATE table_otherbenefits SET Longevity_Status = 1, Hazard_Status = 1, Sublau_Status = 1 WHERE Emp_no = '$emp' OR Longevity_date LIKE '$date_ben%' OR Hazard_date LIKE '$date_ben%' OR Sublau_date LIKE '$date_ben%'");
        header("Location: ViewPayroll.php");
        

}

if(isset($_REQUEST['key']) && $_REQUEST['act'] == 'check'){
    $year = $_REQUEST['year_covered'];
    $month = $_REQUEST['month_covered'];

    mysql_query("UPDATE table_payslip SET verified = 'Yes' WHERE year_covered = '$year' AND month_covered = '$month'");
    mysql_query("UPDATE table_otherbenefits SET Longevity_Status = 1, Hazard_Status = 1, Sublau_Status = 1");
    header("Location: ViewPayroll.php");
}
?>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="card">
                            ...
                            <div style="background-color: #fff; min-height: 80vh;" class="body">
                                ...
                                $datee = $_REQUEST['date'];
                                $date = explode('-', $datee);
                                    $month = date('F', mktime(null, null, null, $date[1], 1));
                                    echo '<p style="font-weight: bold; font-size: 18px; text-align: center;"> Special Payroll ' . $month . ' ' . $date[0] . '</p>';
                                ?>
                                <br><br>
                                <!-- <a class="btn bg-pink btn-md waves-effect" href="ViewEmployeePayslip.php" target="_blank">View Employee Payslip</a><br><br> -->
                                <?php
                                $getDate = explode('-', $_REQUEST['date']);
                                $query3 = mysql_query("SELECT * FROM table_specialpayroll WHERE year_covered = '$getDate[0]' AND month_covered = '$getDate[1]'");
                                ...
                                <table id="example" class="table-bordered stripe row-border order-column" cellspacing="0" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th colspan="9">GSIS</th>
                                            <th colspan="2">PAGIBIG</th>
                                            <th rowspan="2">PHILHEALTH <br> Cont.</th>
                                            <th rowspan="2">LBP <br> Mobile Salary <br> Loan Saver</th>
                                            <th rowspan="2">A/R PLDT</th>
                                            <th rowspan="2">COPEA Dues</th>
                                            <th colspan="2">PMPC</th>
                                            <th rowspan="2">Total <br> Deduction</th> -->
                                            <th colspan="2">Net Amount Due</th>
                                            <th rowspan="2">
                                            <button type="button" class="btn bg-green btn-xs waves-effect" onclick="checkAll();" /><i class="material-icons">done</i></button>
                                            </th>
                                        </tr>
                                        <tr>
                                            <!-- <th>Salary</th>
                                    ...
                                    <tbody>
                                        ...
                                        $dcov = $datee.'-01';
                                        // print_r($dcov);
                                        $query = mysql_query("SELECT * FROM table_specialpayroll WHERE year_covered = '$dateee[0]' AND month_covered = '$dateee[1]'");
                                        $query2 = mysql_query("SELECT * FROM table_otherbenefits WHERE Date_covered = '$dcov'");
...
//                    . "     ON C.division_id = D.division_id");,
                                        ...
                                        while ($row = mysql_fetch_array($query)) {
                                            ...
                                            echo '<td>' . $row['Adj_basicmonsal'] . '</td>';
                                            // echo '<td>' . $row['In_date'] . '</td>';
                                            // echo '<td>' . ($row['Adj_periodcovered'] != 0.00 ? $row['Adj_periodcovered'] : '') . '</td>';
                                            // echo '<td>' . ($row['Pera1'] != 0.00 ? $row['Pera1'] : '') . '</td>';
                                            // echo '<td>' . ($row['Salaries_wagesreg'] != 0.00 ? $row['Salaries_wagesreg'] : '') . '</td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            ...
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '<td>' . ($row['Total_deduction'] != 0.00 ? $row['Total_deduction'] : '') . '</td>';
                                            echo '<td>' . ($row['Net_amount'] != 0.00 ? $row['Net_amount'] : '') . '</td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '</tr>';
                                            ...
                                            // echo '<td>' . ($row['Adj_periodcovered2'] != 0.00 ? $row['Adj_periodcovered2'] : '') . '</td>';
                                            // echo '<td>' . ($row['Pera2'] != 0.00 ? $row['Pera2'] : '') . '</td>';
                                            // echo '<td>' . ($row['Salaries_wagesreg2'] != 0.00 ? $row['Salaries_wagesreg2'] : ''). '</td>';
                                            // echo '<td>' . ($row['Lwop'] != 0.00 ? $row['Lwop'] : ''). '</td>';
                                            // echo '<td>' . ($row['Salaries_withtax'] != 0.00 ? $row['Salaries_withtax'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_rlip'] != 0.00 ? $row['GSIS_rlip'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_ioliprem'] != 0.00 ? $row['GSIS_ioliprem'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_uoliloan'] != 0.00 ? $row['GSIS_uoliloan'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_oli'] != 0.00 ? $row['GSIS_oli'] : '')  . '</td>';
                                            // echo '<td>' . ($row['GSIS_consoloan'] != 0.00 ? $row['GSIS_consoloan'] : ''). '</td>';
                                            // echo '<td>' . ($row['GSIS_calamityloan'] != 0.00 ? $row['GSIS_calamityloan'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_emergloan'] != 0.00 ? $row['GSIS_emergloan'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_educassistloan'] != 0.00 ? $row['GSIS_educassistloan'] : '') . '</td>';
                                            // echo '<td>' . ($row['GSIS_umidca'] != 0.00 ? $row['GSIS_umidca'] : '') . '</td>';
                                            // echo '<td>' . ($row['PAGIBIG_premium'] != 0.00 ? $row['PAGIBIG_premium'] : '') . '</td>';
                                            // echo '<td>' . ($row['PAGIBIG_MPL'] != 0.00 ? $row['PAGIBIG_MPL'] : '') . '</td>';
                                            // echo '<td>' . ($row['PHILH_cont'] != 0.00 ? $row['PHILH_cont'] : ''). '</td>';
                                            // echo '<td>' . ($row['LBP_mobsalloanser'] != 0.00 ? $row['LBP_mobsalloanser'] : '') . '</td>';
                                            // echo '<td>' . ($row['A/R_pldt'] != 0.00 ? $row['A/R_pldt'] : '') . '</td>';
                                            // echo '<td>' . ($row['COPEA_dues'] != 0.00 ? $row['COPEA_dues'] : '') . '</td>';
                                            // echo '<td>' . ($row['PMPC_capital'] != 0.00 ? $row['PMPC_capital'] : '') . '</td>';
                                            // echo '<td>' . ($row['PMPC_loan'] != 0.00 ? $row['PMPC_loan'] : ''). '</td>';
                                            // echo '<td>' . ($row['Total_deduction2'] != 0.00 ? $row['Total_deduction2'] : '') . '</td>';
                                            // echo '<td></td>';
                                            echo '<td>' . ($row['Net_amount2'] != 0.00 ? $row['Net_amount2'] : '') . '</td>';
                                            // echo '<td></td>';
                                            echo '<td>';
                                            if ($row['verified'] == 'Yes') {
                                                echo '<a class="btn bg-light-green btn-xs waves-effect" data-emp="' . $row['Emp_no'] . '" href="EmpPayslipListSP.php?id=' . $row['Emp_no'] . '&date=' . $_REQUEST['date'] . '" target="_blank">View Payslip</a>';
                                                $date_ben = explode('-',$row['year_covered'].'-'.$row['month_covered'] . '-01');
                                                $date_ben = $date_ben[0].'-'.$date_ben[1];

                                                mysql_query("UPDATE table_otherbenefits SET Longevity_Status = 1, Hazard_Status = 1, Sublau_Status = 1 WHERE Emp_no = ".$row['Emp_no']." AND Longevity_date LIKE '$date_ben%' AND Hazard_date LIKE '$date_ben%' AND Sublau_date LIKE '$date_ben%'");
                                            }else{
                                                    echo '<a class="confirmme btn bg-green btn-xs waves-effect" data-toggle="tooltip" data-placement="top" title="Confirm" name="confirm" data-key="pick" data-emp="' . $row['Emp_no'] . '" data-date="' . $row['In_date'] . '" date-benefits="' . $row['year_covered'] . '-' . $row['month_covered'] . '-01" target="_blank"><i class="material-icons">done</i></a>&nbsp;';
                                            }
                                            echo '</td>';
                                            echo '</tr>';
                                        }
    ...
    <script language="javascript">
        ...
        $(document).ready(function () {
            ...
            $('#example').on('click', 'a.confirmme', function (e) {
                ...
                var for_benefits = $(this).attr('date-benefits');

                swal({
                    title: "Are you sure?",
                    text: "It will change the status to verified",
                    type: "warning",
                ...
                function () {
                    $.ajax({
                        type: 'POST',
                        url: 'ViewSpecialPayroll.php?emp=' + emp + '&date=' + date + '&date_ben='+ for_benefits + '&key='+ key +'&act=app',
                        success: function () {
                            $('#example').attr("id");

                        
                                a.closest('td').html('<a class="btn bg-light-green btn-xs waves-effect" data-emp="'+ emp +'" href="EmpPayslipList.php?id='+ emp +'" target="_blank">View Payslip</a>');
                                swal("Successful!", "You can now view his/her payslip", "success");
                        }
                    });
        ...
        function checkAll(){
                ...
                var year_covered = <?php echo $getDate[0]; ?>;
                var month_covered = <?php echo $getDate[1]; ?>;
                ...
                function () {
                    $.ajax({
                        type: 'POST',
                        url: 'Checkall.php?&type=special&year='+ year_covered +'&month='+ month_covered,
                        success: function (result)
                        {
                            swal({title: "Successful!", text: "The page will refresh... \n\n © "+n+" POPCOM.", type: "success"},
                                function(){
                                    location.reload();
                                }
                            );
