                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="card">
                            <div class="header">
                                ...
                                <h2>Potential Assessment List</h2>
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" id="bodyy" class="body">
                                <div style="height: 730px; right: 0; width: auto; padding: 40px;">
                                    <table id="qualificationTable" class="table table-bordered table-striped table-hover dataTable js-basic-example">
                                        <thead>
                                            <tr>
                                                <th>Last Name</th>
                                                <th>First Name</th>
                                        ...
                                        <tbody >
                                            <?php
                                            $query0 = mysql_query("SELECT * FROM table_empinfo WHERE Employee_Id = " . $_SESSION['INTRA_EMP_ID']);
                                            $row0 = mysql_fetch_array($query0);
                                            $query = mysql_query("SELECT table_empinfo.*, table_empinfo.Id as PDS_ID, table_empinfo.Position as empPos, table_position.*, table_qualification.*, table_qualification.Position as quaPos, table_postingstatus.* FROM table_empinfo"
                                                    . "           LEFT JOIN table_position"
                                                    . "                 ON table_position.position_id = table_empinfo.Position"
                                                    . "           LEFT JOIN table_qualification"
                                                    . "                 ON table_qualification.Pds_Id = table_empinfo.Id"
                                                    . "           LEFT JOIN table_postingstatus"
                                                    . "                 ON table_postingstatus.Pds_Id = table_empinfo.Id WHERE table_empinfo.Office_id = " . $row0['Office_id']);

                                            while ($row = mysql_fetch_array($query)) {
                                                

                                                if ($row['Potential_status'] == 1) {
                                                    if (isset($row['quaPos'])) {
                                                        echo '<tr style="font-size: 13px;">';
                                                        echo '<td>' . ucwords(strtolower($row['Lastname'])) . '</td>';
                                                        echo '<td>' . ucwords(strtolower($row['Firstname'])) . '</td>';
                                                        echo '<td>' . ucwords(strtolower($row['Middlename'])) . '</td>';
                                                        echo '<td><span data-pos="' . $row['quaPos'] . '">' . $row['quaPos'] . '</span></td>';
                                                        echo '<td>' . $row['Division'] . '</td>';
                                                        echo '<td>';
                                                        echo '  <a href="PotentialAssessmentForm.php?id=' . $row['PDS_ID'] . '" class="btn bg-teal btn-xs waves-effect" target="_blank">Assess</a>';
                                                        echo '  <a href="ViewPotentialAssessmentForm.php?id=' . $row['PDS_ID'] . '" class="btn bg-cyan btn-xs waves-effect" target="_blank">View</a>';
                                                        echo '</td>';
                                                        echo '</tr>';
                                                    } else {
                                                        echo '<tr style="font-size: 13px;">';
                                                        ...
                                                        echo '<td>' . ucwords(strtolower($row['Firstname'])) . '</td>';
                                                        echo '<td>' . ucwords(strtolower($row['Middlename'])) . '</td>';
                                                        echo '<td>Not yet assigned</td><td></td>';
                                                        echo '<td>';
                                                        echo '  <a href="PotentialAssessmentForm.php?id=' . $row['PDS_ID'] . '" class="btn bg-teal btn-xs waves-effect" target="_blank">Assess</a>';
                                                        echo '  <a href="ViewPotentialAssessmentForm.php?id=' . $row['PDS_ID'] . '" class="btn bg-cyan btn-xs waves-effect" target="_blank">View</a>';
                                                        echo '</td>';
