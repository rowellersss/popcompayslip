...
if (isset($_REQUEST['id']) && $_REQUEST['act'] == 'set') {
    ...
    $query4 = mysql_query("select * from table_empinfo where Employee_Id = '$id2'");
    $row4 = mysql_fetch_assoc($query4);
    $name = ucfirst($row4['Firstname']) . ' ' . ucfirst($row4['Lastname']);
    
    $date = date('Y-m-d');
    mysql_query("Update table_servicereq set Status = 'Approved', DateApproved = '$date', Ind = '1', ApprovedBy = '$name' where Id = '$id'");
}
?>
                        ...
                        <div class="card">
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" class="body">
                                <table class="table table-bordered table-striped table-hover dataTable js-basic-example">
                                    ...
                                    <tbody >
                                        <?php
                                        $id3 = $_SESSION['INTRA_EMP_ID'];
                                        $query3 = mysql_query("select * from table_empinfo where Employee_Id = '$id3'");
                                        $row3 = mysql_fetch_assoc($query3);
                                        $dur = $row3['DivUnitRPO'];
                                        $query2 = mysql_query("select * from table_servicereq where Status = 'Approval' AND DivUnitRPO = '$dur' order by Id asc");
                                        while ($row = mysql_fetch_array($query2)) {
                                            $date = date('M d, Y', strtotime($row['DateofReq']));
                                            echo '<tr><td>' . ucwords($row['ReqBy']) . '</td><td>' . $date . '</td><td>' . $row['DivUnitRPO'] . '</td>'
                                            . '<td style="max-width: 200px;">' . $row['DescOfReq'] . '</td><td>' . $row['Status'] . '</td>'
                                            . '<td><a href="ManageServiceReq2.php?id=' . $row['Id'] . '&act=set"><i class="material-icons">thumb_up</i></a></tr>';
                                        }
                                        ?>
