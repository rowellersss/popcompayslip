...
if (isset($_POST['submit'])) {
    $date = $_POST['date'];
    $_SESSION['INTRA_DATE_OB'] = $date;
    header("Location: ViewOtherBenefits.php");
}
?>
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="card">
                            ...
                            <div style="background-color: #fff; min-height: 80vh;" class="body">
                                <br><br>
                                <form action="OBList.php" method="post">
                                    <label for="date">Choose a Date: </label>
                                    <select name="date" id="date">
                                        <option>- - -</option>
                                        <?php
                                        $query = mysql_query("SELECT * FROM table_otherbenefits GROUP BY Date_covered");
                                        while ($row = mysql_fetch_array($query)) {   
                                            echo '<option value="' . $row['Date_covered'] . '">' . date('F, Y', strtotime($row['Date_covered'])) . '</option>';
                                        }
                                        ?>
                                    </select>&nbsp;&nbsp;&nbsp;
                                    <input type="submit" name="submit" class="btn bg-cyan btn-md waves-effect" value="Submit">
                                </form>
                            </div>
