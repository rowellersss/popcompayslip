...
if (isset($_POST['process'])) {
    if ($_POST['stat'] == 'Finished') {
        header("Location: ManageServiceReqFin.php");
    }
}
...
if (isset($_REQUEST['Id']) && $_REQUEST['act'] == 'app') {
    ...
    $id = $_REQUEST['Id'];
    mysql_query("Update table_servicereq set Ind = '1', Status = 'Approval' where Id = '$id'");
}

if (isset($_REQUEST['Id']) && $_REQUEST['act'] == 'ong') {

    $id = $_REQUEST['Id'];
    $idd = $_SESSION['INTRA_EMP_ID'];
    $sel = mysql_query("select * from table_empinfo where Employee_Id = '$idd'");
    $row2 = mysql_fetch_assoc($sel);
    $name1 = ucfirst($row2['Firstname']) . " " . ucfirst($row2['Lastname']);
    mysql_query("Update table_servicereq set Status = 'Ongoing', AcTakenBy = '$name1' where Id = '$id'");
}

if (isset($_POST['reports'])) {
    header("Location: ManageServiceReqDone.php");
}
?>
                        ...
                        <div class="card">
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" class="body">
                                <div style="min-height: 77vh; right: 0; width: auto; padding: 40px;">
                                    <form action="ManageServiceReq.php" method="post">
                                        <div class="row clearfix">
                                            ...
                                            <div class="col-sm-2">
                                                <b>Div/Unit/RPO</b>
                                                <div class="form-group">
                                                    <select name="div" class="form-control show-tick">
                                            ...
                                            <div class="col-sm-2">
                                                <b>Status</b>
                                                <div class="form-group">
                                                    <select name="stat" class="form-control show-tick">
                                                        <option value="">-- Please select --</option>
                                            ...
                                            <div class="col-sm-2" style="margin-top: 10px;">
                                                <input type="submit" name="process" id="click" class="btn bg-light-green waves-effect" value="Process">&nbsp;&nbsp;&nbsp;
                                                <input type="submit" name="reports" class="btn bg-light-blue waves-effect" value="Reports">
                                            </div>
                                        </div>
                                    </form><br>
                                            ...
                                            <tbody >
                                                ...
                                                if (isset($_POST['process'])) {
                                                    ...
                                                    if (!empty($d1) && !empty($d2) && empty($d3) && empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND DateofReq BETWEEN '$start' AND '$end' order by Id asc");
                                                    }

                                                    if (empty($d1) && empty($d2) && !empty($d3) && empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND DivUnitRPO = '$d3' order by Id asc");
                                                    }

                                                    if (empty($d1) && empty($d2) && empty($d3) && !empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND Status = '$d4' order by Id asc");
                                                    }

                                                    if (!empty($d1) && !empty($d2) && !empty($d3) && !empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND DateofReq BETWEEN '$start' AND '$end' AND DivUnitRPO = '$d3' AND Status = '$d4' order by Id asc");
                                                    }

                                                    if (!empty($d1) && !empty($d2) && !empty($d3) && empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND DateofReq BETWEEN '$start' AND '$end' AND DivUnitRPO = '$d3' order by Id asc");
                                                    }

                                                    if (empty($d1) && empty($d2) && !empty($d3) && !empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND DivUnitRPO = '$d3' AND Status = '$d4' order by Id asc");
                                                    }

                                                    if (!empty($d1) && !empty($d2) && empty($d3) && !empty($d4)) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' AND DateofReq BETWEEN '$start' AND '$end' AND Status = '$d4' order by Id asc");
                                                    }

                                                    if (empty($_POST['start']) && empty($_POST['end']) && empty($_POST['div']) && empty($_POST['stat'])) {
                                                        $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' order by Id asc");
                                                    }
                                                }
                                                ...
                                                $from = "2016-09-01";
                                                $to = "2016-09-21";
                                                // AND DateofReq BETWEEN '$from' AND '$to' AND DivUnitRPO = 'ITDMU'
                                                if (!isset($_POST['start']) && !isset($_POST['end']) && !isset($_POST['div']) && !isset($_POST['stat'])) {
                                                    $query2 = mysql_query("select * from table_servicereq where Status != 'Finished' order by Id asc");
                                                }
                                                ...
                                                while ($row = mysql_fetch_array($query2)) {
                                                    ...
                                                    if ($row['Status'] == 'Ongoing') {
                                                        ...
                                                        $atb = 'Not yet assigned';
                                                    }
                                                    if ($row['AcTakenBy'] == '') {
                                                        $atb = 'Not yet assigned';
                                                    }

                                                    if ($row['Status'] == 'Approved' || $row['Status'] == 'Ongoing') {
                                                        echo '<tr><td>' . ucwords($row['Id']) . '</td><td>' . ucwords($row['ReqBy']) . '</td><td>' . $date . '</td><td>' . $row['TimeOfReq'] . '</td><td>' . $row['DivUnitRPO'] . '</td>'
                                                        . '<td style="max-width: 200px;">' . $row['DescOfReq'] . '</td><td>' . $atb . '</td><td style="color:' . $color . '"><b>' . $row['Status'] . '</b></td>'
                                                        . '<td><a title="View Request Form" class="remove" href="../RequestForm.php?Id=' . $row['Id'] . '&act=set"><i class="material-icons">visibility</i></a>&nbsp;&nbsp;&nbsp;&nbsp;'
                                                        . '<a title="Input Action Taken" href="ActionTaken.php?Id=' . $row['Id'] . '&act=set"><i class="material-icons">visibility</i></a>&nbsp;&nbsp;&nbsp;&nbsp;'
                                                        . '<a title="Ongoing" href="ManageServiceReq.php?Id=' . $row['Id'] . '&act=ong"><i class="material-icons">cached</i></a></td></tr>';
                                                        ...
                                                        echo '<tr><td>' . ucwords($row['Id']) . '</td><td>' . ucwords($row['ReqBy']) . '</td><td>' . $date . '</td><td>' . $row['TimeOfReq'] . '</td><td>' . $row['DivUnitRPO'] . '</td>'
                                                        . '<td style="max-width: 200px;">' . $row['DescOfReq'] . '</td><td>' . $atb . '</td><td style="color: orange;"><b>' . $row['Status'] . '</b></td>'
                                                        . '<td><a title="View Request Form" class="remove" href="../RequestForm.php?Id=' . $row['Id'] . '&act=set"><i class="material-icons">visibility</i></a>&nbsp;&nbsp;&nbsp;&nbsp;'
                                                        . '<a title="Input Action Form" href="ActionTaken.php?Id=' . $row['Id'] . '&act=set"><i class="material-icons">visibility</i></a>&nbsp;&nbsp;&nbsp;&nbsp;'
                                                        . '<a title="For Approval" href="ManageServiceReq.php?Id=' . $row['Id'] . '&act=app"><i class="material-icons">thumb_up</i></a>&nbsp;&nbsp;&nbsp;&nbsp;'
                                                        . '<a title="Ongoing" href="ManageServiceReq.php?Id=' . $row['Id'] . '&act=ong"><i class="material-icons">cached</i></a></td></tr>';
                                                    }
                                                }

//                                                //<a href="Survey.php?Id=' . $row['Id'] . '&act=set"><i class="material-icons">visibility</i></a>
                                                ?>
                                            </tbody>
                                        </table>
                                    </div>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#qualificationTable').on('click', 'a.forapp', function (e) {
                    e.preventDefault();
                    var a = $(this);
                    ...
                    swal({
                        title: "Are you sure?",
                        text: "Status will be updated to approval!",
                        type: "warning",
