...
if (isset($_POST['btnBack'])) {
    header("Location: LeaveApplicationList.php");
}

$pdsid = $_SESSION['INTRA_ID'];
$query = mysql_query("SELECT * FROM table_empinfo A"
        . "             LEFT JOIN table_position B"
        . "                 ON A.Position = B.position_id"
        . "             LEFT JOIN table_division C"
        . "                 ON B.division_id = C.division_id"
        . "             LEFT JOIN table_salarymatrix D"
        . "                 ON B.salary_id = D.salarymatrix_id WHERE Id = '$pdsid'");
...
if (isset($_REQUEST['act']) && $_REQUEST['act'] == 'create') {
    ...
    if (isset($_POST['dof'])) {
        $dof = mysql_real_escape_string($_POST['dof']);
    } else {
        $dof = '';
    }
    if (isset($_POST['textVaca'])) {
        $textVaca = mysql_real_escape_string($_POST['textVaca']);
    } else {
        $textVaca = '';
    }
    if (isset($_POST['textSick'])) {
        $textSick = mysql_real_escape_string($_POST['textSick']);
    } else {
        $textSick = '';
    }
    if (isset($_POST['texticvl'])) {
        $texticvl = mysql_real_escape_string($_POST['texticvl']);
    } else {
        $texticvl = '';
    }
    if (isset($_POST['texticsl'])) {
        $texticsl = mysql_real_escape_string($_POST['texticsl']);
    } else {
        $texticsl = '';
    }
    if (isset($_POST['texticsl1'])) {
        $texticsl1 = mysql_real_escape_string($_POST['texticsl1']);
    } else {
        $texticsl1 = '';
    }

    if (isset($_POST['type'])) {
        $d1 = mysql_real_escape_string($_POST['type']);
    } else {
        $d1 = '';
    }
    if (isset($_POST['vaca'])) {
        if ($_POST['vaca'] == 'Others1') {
            $d2 = mysql_real_escape_string($textVaca);
        } else {
            $d2 = 'To seek employment';
        }
    } else {
        $d2 = '';
    }
    if (isset($_POST['sick'])) {
        if ($_POST['sick'] == 'Others2') {
            $d3 = mysql_real_escape_string($textSick);
        } else {
            $d3 = 'MATERNITY';
        }
    } else {
    ...
    if (isset($_POST['icvl'])) {
        if ($_POST['icvl'] == 'Abroad') {
            $d4 = mysql_real_escape_string($texticvl);
        } else {
            $d4 = 'Within the Philippines';
        }
    } else {
        $d4 = '';
    }
    if (isset($_POST['icsl'])) {
        if ($_POST['icsl'] == 'Hospital') {
            $d5 = mysql_real_escape_string('HOS * ' . $texticsl1);
        } elseif ($_POST['icsl'] == 'Home') {
            $d5 = 'HOM * ' . $texticsl;
        }
    } else {
        $d5 = '';
    ...
    if (isset($_POST['wd'])) {
        $d6 = mysql_real_escape_string($_POST['wd']);
    } else {
        $d6 = 0;
    }
    if (isset($_POST['inclFr'])) {
        $d7 = mysql_real_escape_string(date('Y-m-d', strtotime($_POST['inclFr'])));
    } else {
        $d7 = '';
    }
    if (isset($_POST['inclTo'])) {
        $d8 = mysql_real_escape_string(date('Y-m-d', strtotime($_POST['inclTo'])));
    } else {
        $d8 = '';
    }
    if (isset($_POST['req'])) {
        $d9 = mysql_real_escape_string($_POST['req']);
    } else {
        $d9 = '';
    }

    $sel = mysql_query("SELECT MAX(Transaction_no) as maxTran FROM table_leaveapplication");
    $row2 = mysql_fetch_assoc($sel);
    $nextTran = $row2['maxTran'] + 1;
    $tracno = str_pad($nextTran, 4, 0, STR_PAD_LEFT);

    //mysql_query("INSERT INTO table_leaveapplication VALUES (NULL,'$pdsid','$dof','$d1','$d2$d3','$d4$d5','$d6','$d7','$d8','$d9','$tracno')");

    $_SESSION['INTRA_INDICATOR'] = 'true';
    header("Location: ApplicationForLeave.php");
}
?>
    ...
    <head>
        ...
        <script>
            ...
            $(document).ready(function () {
                $('#radio2,#radio3,#radio5,#radio6,#vacaDiv,#sickDiv,#wd,#text1,#text2,#text3,#text4,#text5,#hrr').hide();
                $('#vacaLeave').click(function () {
                    $('#radio2,#radio3,#vacaDiv,#wd,#hrr').show();
                    $('#radio5,#radio6,#sickDiv').hide();
                });
                $('#sickLeave').click(function () {
                    $('#radio5,#radio6,#sickDiv,#wd,#hrr').show();
                    $('#radio2,#radio3,#vacaDiv').hide();
                });
                $('#vaca2').click(function () {
                    $('#text1').show();
                });
                $('#vaca').click(function () {
                    $('#text1').hide();
                });
                $('#sick2').click(function () {
                    $('#text2').show();
                });
                $('#sick').click(function () {
                    $('#text2').hide();
                });
                $('#icvl2').click(function () {
                    $('#text3').show();
                });
                $('#icvl').click(function () {
                    $('#text3').hide();
                ...
                <div class="row clearfix">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="card">
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" class="body">
                                <div style="min-height: 77vh;right: 0; width: auto; padding: 40px;">
                                    <h3>DETAILS OF APPLICATION</h3><br><br><br>

                                    <form action="ApplicationForLeave.php" method="post" id="sweet">
                                        <div style="width: 70%; margin:0 auto;">

                                            <div class="well" style="background-color: white; height: 80px;">
                                                <div class="row clearfix">
                                                    <div class="col-sm-2">
                                                        <div class="form-group">
                                                            <p style="font-size: 13.5px; font-weight: bold; padding-top: 10px; margin-left: 30px;">Date of filing:</p>
                                                        </div>
                                                    </div>
                                                    <div class="col-sm-3">
                                                        <div class="form-group">
                                                            <div class="form-line">
                                                                <input type="text" class="datepicker form-control" id="datePicker" name="dof" placeholder="Please choose a date...">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="well" style="background-color: white; padding-left: 50px;">

                                                <p style="font-weight: bold;">a.) Type of leave</p>

                                                <input name="type" type="radio" id="vacaLeave" value="Vacation" class="with-gap radio-col-orange" />
                                                <label for="vacaLeave"> Vacation</label>
                                                <input name="type" type="radio" id="sickLeave" value="Sick" class="with-gap radio-col-orange" />
                                                <label for="sickLeave"> Sicks</label><br><br>

                                                <div>   
                                                    <!----------------------VACATION---------------------->
                                                    <div id="radio2">
                                                        <input name="vaca" type="radio" id="vaca" value="Employment" class="with-gap radio-col-orange" />
                                                        <label for="vaca"> To seek employment</label>
                                                    </div>
                                                    <div id="radio3">
                                                        <input name="vaca" type="radio" id="vaca2" value="Others1" class="with-gap radio-col-orange" />
                                                        <label for="vaca2"> Others (Specify)</label>
                                                        <div id="text1" class="form-group" style="width: 300px;">
                                                            <div class="form-line">
                                                                <input type="text" name="textVaca" class="form-control" placeholder="Specify"/>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <!----------------------SICK---------------------->
                                                    <div id="radio5">
                                                        <input name="sick" type="radio" id="sick" value="Maternity" class="with-gap radio-col-orange" />
                                                        <label for="sick"> Maternity</label>
                                                    </div>
                                                    <div id="radio6">
                                                        <input name="sick" type="radio" id="sick2" value="Others2" class="with-gap radio-col-orange" />
                                                        <label for="sick2"> Others (Specify)</label>
                                                        <div id="text2" class="form-group" style="width: 300px;">
                                                            <div class="form-line">
                                                                <input type="text" name="textSick" class="form-control" placeholder="Specify"/>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    <div id="vacaDiv">
                                                        ...
                                                        <p style="font-size: 15px; font-weight: bold;">b.) Where leave will be spent</p><br>
                                                        <p style="font-size: 15px; font-weight: bold;">In case of vacation leave</p>
                                                        <div id="radio7">
                                                            <input name="icvl" type="radio" id="icvl" value="Within" class="with-gap radio-col-orange" />
                                                            <label for="icvl"> Within the Philippines</label>
                                                        </div>
                                                        <div id="radio8">
                                                            <input name="icvl" type="radio" id="icvl2" value="Abroad" class="with-gap radio-col-orange" />
                                                            <label for="icvl2"> Abroad (Specify)</label>
                                                            <div id="text3" class="form-group" style="width: 300px;">
                                                                <div class="form-line">
                                                                    <input type="text" name="texticvl" class="form-control" placeholder="Specify"/>
                                                                </div>
                                                            </div>
                                                        </div><br>
                                                    </div>
                                                    <div id="sickDiv">
                                                        ...
                                                        <p style="font-size: 15px; font-weight: bold;">In case of vacation leave</p>
                                                        <div id="radio7">
                                                            <input name="icsl" type="radio" id="icsl" value="Hospital" class="with-gap radio-col-orange" />
                                                            <label for="icsl"> In Hospital (Specify)</label>
                                                            <div id="text4" class="form-group" style="width: 300px;">
                                                                <div class="form-line">
                                                                    <input type="text" name="texticsl1" class="form-control" placeholder="Specify"/>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div id="radio8">
                                                            <input name="icsl" type="radio" id="icsl2" value="Home" class="with-gap radio-col-orange" />
                                                            <label for="icsl2"> At home (Specify)</label>
                                                            <div id="text5" class="form-group" style="width: 300px;">
                                                                <div class="form-line">
                                                                    <input type="text" name="texticsl" class="form-control" placeholder="Specify"/>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div id="wd" class="well" style="background-color: white; padding-left: 50px;">
                                                <div>
                                                    <p style="font-size: 15px; font-weight: bold;">c.) Number of working days applied for:</p>
                                                    <div class="form-group" style="width: 200px;">
                                                        <div class="form-line">
                                                            <input type="text" name="wd" class="form-control" placeholder="Input here..."/>
                                                        </div>
                                                    </div>
                                                    <p style="font-size: 15px; font-weight: bold;">Inclusive dates</p>

                                                    <div class="row clearfix">
                                                        <div class="col-sm-2">
                                                            <div class="form-group">
                                                                <div class="form-line">
                                                                    <input type="text" class="datepicker form-control" name="inclFr" placeholder="From">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="col-sm-2">
                                                            <div class="form-group">
                                                                <div class="form-line">
                                                                    <input type="text" class="datepicker form-control" name="inclTo" placeholder="To">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <p style="font-size: 15px; font-weight: bold;">d.) Commutation</p>
                                                    <input name="req" type="radio" id="req" value="Requested" class="with-gap radio-col-orange" />
                                                    <label for="req" >Requested</label>
                                                    <input name="req" type="radio" id="nreq" value="Not Requested" class="with-gap radio-col-orange" />
                                                    <label for="nreq"> Not Requested</label><br><br><br>
                                                    <a href="ApplicationForLeave.php?act=create" class="Screate btn bg-indigo waves-effect">Create</a>
                                                    <input type="submit" name="btnBack" value="Back" class="btn bg-blue waves-effect">

                                                </div>



<!--                                    <input type="radio" name="type" id="vacaLeave" value="Vacation"> VACATION<br>
<input type="radio" name="vaca" id="vaca" value="Employment"> To seek employment<br>
<input type="radio" name="vaca" id="vaca2" value="Others1"> Others (Specify)<br>
<input type="text" name="textVaca" placeholder="Specify"><br><br>

<input type="radio" name="type" id="sickLeave" value="Sick"> SICK<br>
<input type="radio" name="sick" id="sick" value="Maternity"> MATERNITY<br>
<input type="radio" name="sick" id="sick2" value="Others2"> OTHERS (Specify)<br>
<input type="text" name="textSick" placeholder="Specify"
                                    <br><br>
                                    b.) WHERE LEAVE WILL BE SPENT<br><br>
                                    (1) IN CASE OF VACATION LEAVE<br>
                                    <input type="radio"> Within the Philippines<br>
                                    <input type="radio"> Abroad (Specify)<br>
                                    <input type="text" placeholder="Specify"><br><br>
                                    (2) IN CASE OF SICK LEAVE<br>
                                    <input type="radio"> In Hospital (Specify)
                                    <input type="text" placeholder="Specify"><br>
                                    <input type="radio"> At home (Specify)
                                    <input type="text" placeholder="Specify"><br><br>>
                                    c.) NUMBER OF WORKING DAYS APPLIED FOR:<br>
                                    <input type="number"><br>
                                    INCLUSIVE DATES<br>
                                    TO <input type="date"> - FROM <input type="date"><br><br>
                                    d.) COMMUTATION<br><br>
                                    <input type="radio"> Requested<br>
                                    <input type="radio"> Not Requested<br>-->
                                            </div>
                                        </div>
                    ...
                    if (isset($_SESSION['INTRA_INDICATOR']) && $_SESSION['INTRA_INDICATOR'] == 'true') {
                        ...
                        <script>
                            $(document).ready(function () {
                                swal("Successful!", "Application for leave has been successfully created!", "success");
                            })
                        </script>
                        <?php
                    }
