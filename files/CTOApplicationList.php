...
if (isset($_POST['btnCreate'])) {
    header("Location: ApplicationForCTO.php");
}
                        ...
                        <div class="card">
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" class="body">
                                <div style="min-height: 77vh;right: 0; width: auto; padding: 40px;">
                                    <form action="CTOApplicationList.php" method="post">
                                        <input type="submit" name="btnCreate" value="Create New Application for CTO"  class="btn bg-indigo waves-effect"><br><br><br>
                                    </form>
                                    <?php
                                    $query = mysql_query("SELECT * FROM table_ctoapplication WHERE Pds_Id = '$pdsid' ORDER BY Date_offiling desc");
                                    while ($row = mysql_fetch_array($query)) {
                                        echo '<a href="ViewApplicationForCTO.php?trac_no=' . md5($row['Transaction_no']) . '" target="_blank">' . 'Document No. ' . $row['Transaction_no'] . ' / ' . $row['Date_offiling'] . '</a><br><br>';
                                    }
                                    ?>
                                </div>
                            </div>
