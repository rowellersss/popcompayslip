...
if (isset($_REQUEST['emp']) && $_REQUEST['act'] == 'app') {
    ...
    $key = $_REQUEST['key'];
        ...
        mysql_query("UPDATE table_payslip SET verified = 'Yes' WHERE Emp_no = '$emp' AND Adj_periodcovered = '$date'");
                        ...
                        <div class="card">
                            ...
                            <div style="background-color: #fff; min-height: 80vh;" class="body">
                                ...
                                $date = explode('-', $datee);
                                    ...
                                    echo '<p style="font-weight: bold; font-size: 18px; text-align: center;">Salary ' . $month . ' ' . $date[0] . '</p>';
                                ?>
                                <br><br>
                                ...
                                $query3 = mysql_query("SELECT * FROM table_payslip WHERE year_covered = '$getDate[0]' AND month_covered = '$getDate[1]'");
                                ...
                                <table id="example" class="table-bordered stripe row-border order-column" cellspacing="0" width="100%">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th rowspan="2">
                                            <button type="button" class="btn bg-green btn-xs waves-effect approve"/><i class="material-icons">done</i></button>
                                            </th>
                                    ...
                                    <tbody class="body">
                                        ...
                                        // $dcov = implode('-', $dateee);
                                        $dcov = $datee.'-01';
                                        $query = mysql_query("SELECT * FROM table_payslip WHERE year_covered = '$dateee[0]' AND month_covered = '$dateee[1]'");
...
//                    . "     ON C.division_id = D.division_id");,
                                        ...
                                        while ($row = mysql_fetch_array($query)) {
                                            ...
                                            // echo '<td>' . $row['Salaries_wagesreg'] . '</td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            // echo '<td></td>';
                                            ...
                                            // echo '<td>' . $row['Salaries_wagesreg2'] . '</td>';
                                            // echo '<td>' . $row['Salaries_withtax'] . '</td>';
                                            // echo '<td>' . $row['GSIS_rlip'] . '</td>';
                                            // echo '<td>' . $row['GSIS_ioliprem'] . '</td>';
                                            // echo '<td>' . $row['GSIS_uoliloan'] . '</td>';
                                            // echo '<td>' . $row['GSIS_oli'] . '</td>';
                                            // echo '<td>' . $row['GSIS_consoloan'] . '</td>';
                                            // echo '<td>' . $row['GSIS_emergloan'] . '</td>';
                                            // echo '<td>' . $row['GSIS_policyloanreg'] . '</td>';
                                            // echo '<td>' . $row['GSIS_policyloanoptional'] . '</td>';
                                            // echo '<td>' . $row['GSIS_educassistloan'] . '</td>';
                                            // echo '<td>' . $row['GSIS_umidca'] . '</td>';
                                            // echo '<td>' . $row['PAGIBIG_premium'] . '</td>';
                                            // echo '<td>' . $row['PAGIBIG_MP2'] . '</td>';
                                            // echo '<td>' . $row['PAGIBIG_MPL'] . '</td>';
                                            // echo '<td>' . $row['PAGIBIG_calamityloan'] . '</td>';
                                            // echo '<td>' . $row['PHILH_cont'] . '</td>';
                                            // echo '<td>' . $row['PHILH_arrears'] . '</td>';
                                            // echo '<td>' . $row['PMPC_capital'] . '</td>';
                                            // echo '<td>' . $row['PMPC_loan'] . '</td>';
                                            // echo '<td>' . $row['COPEA_dues'] . '</td>';
                                            // echo '<td>' . $row['LBP_salaryloanreg'] . '</td>';
                                            // echo '<td>' . $row['LBP_mobsalloanser'] . '</td>';
                                            ...
                                            echo '<td>';
                                            if ($row['verified'] == 'Yes') {
                                                echo '<a class="btn bg-light-green btn-xs waves-effect" data-emp="' . $row['Emp_no'] . '" href="EmpPayslipList.php?id=' . $row['Emp_no'] . '&date=' . $_REQUEST['date'] . '" target="_blank">View Payslip</a>';
                                                ...
                                                mysql_query("UPDATE table_otherbenefits SET Longevity_Status = 1, Hazard_Status = 1, Sublau_Status = 1 WHERE Emp_no = ".$row['Emp_no']." AND Longevity_date LIKE '$date_ben%' AND Hazard_date LIKE '$date_ben%' AND Sublau_date LIKE '$date_ben%'");
                                                    ...
                                                    echo '<a class="confirmme btn bg-green btn-xs waves-effect" data-toggle="tooltip" data-placement="top" title="Confirm" name="confirm" data-key="pick" data-emp="' . $row['Emp_no'] . '" data-date="' . $row['Adj_periodcovered'] . '" date-benefits="' . $row['year_covered'] . '-' . $row['month_covered'] . '-01" target="_blank"><i class="material-icons">done</i></a>';
                                                    // echo '<a class="btn bg-teal btn-xs waves-effect" data-toggle="tooltip" data-placement="top" title="Edit" href="Editpayroll.php?emp=' . $row['Emp_no'] . '&date=' . $row['Adj_periodcovered'] . '" target="_blank"><i class="material-icons">mode_edit</i></a>';
                                            }
                                            echo '</td>';
                                            echo '</tr>';
                                        }
    ...
    <script language="javascript">
        ...
        $(document).ready(function () {
            ...
            $('#example').on('click', 'a.confirmme', function (e) {
                ...
                var dcov = <?php echo $_REQUEST['date']; ?>;
                ...
                function () {
                    $.ajax({
                        type: 'POST',
                        url: 'ViewPayroll.php?emp=' + emp + '&date=' + date + '&date_ben='+ for_benefits + '&key='+ key +'&act=app',
                        success: function () {
                            $('#example').attr("id");

                        
                                a.closest('td').html('<a class="btn bg-light-green btn-xs waves-effect" data-emp="'+ emp +'" href="EmpPayslipList.php?id='+ emp +' &date='+ dcov +' target="_blank">View Payslip</a>');
            ...
            $('.approve').click(function(e){
                ...
                function () {
                    $.ajax({
                        type: 'POST',
                        url: 'Checkall.php?&type=not_special'+'&year='+ year_covered +'&month='+ month_covered,
        ...
        //         swal({
        //             title: "Are you sure?",
        //             text: "It will change all the status to verified",
        //             type: "warning",
        //             showCancelButton: true,
        //             confirmButtonColor: "#4CAF50",
        //             confirmButtonText: "Yes",
        ...
        //             $.ajax({
        //                 type: 'POST',
        //                 url: 'Checkall.php?&year='+ year_covered +'&month='+ month_covered,
        //                 success: function () {
        //                         $(this).closest('tr').html('<th rowspan="2">Action</th>');
        //                         swal("Successful!", "You can now view all payslips", "success");
        //                 }
        //             });
