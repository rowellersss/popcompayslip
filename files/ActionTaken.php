...
if (isset($_POST['back'])) {
    header("Location: ManageServiceReq.php");
}
...
if (isset($_POST['submit'])) {
    ...
    $query2 = mysql_query("Select * from table_servicereq where Id = '$id'");
    $row = mysql_fetch_assoc($query2);
    $dateStart = $row['DateApproved'];
    $date = date('Y-m-d', strtotime($d3));
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["at"])) {
            $atErr = "Action Taken is required!";
        } else {
            $at = $_POST["at"];
        }

        if (empty($_POST["date"])) {
            $dateErr = "Date Finished is required!";
        } else {
            $date = $_POST["date"];
        }

        if (empty($atErr) && empty($dateErr)) {
            if ($days <= 1) {
                $cat = 'Minor';
            } else {
                $cat = 'Major';
            ...
            $sql = mysql_query("Select * from table_empinfo where Employee_Id = '$idd'");
            $row5 = mysql_fetch_assoc($sql);
            $name1 = ucfirst($row5['Firstname']) . " " . ucfirst($row5['Lastname']);

            mysql_query("Update table_servicereq set ActionTaken = '$d1', AcTakenBy = '$name1', DateFinished = '$date', Category = '$cat', Status = 'Finished'  where Id = '$id'");
            header('Location: ../RequestForm.php?Id=' . $id . '&act=set');
        } elseif (!empty($atErr) || !empty($dateErr)) {
                        ...
                        <div class="card">
                            ...
                            <div style="min-height: 80vh; background-color: #fff;" class="body">
                                <div style="height: 730px; right: 0; width: auto; padding: 40px;">
                                    <form method="post" action="<?php echo 'ActionTaken.php?Id=' . $_GET['Id']; ?>">


                                        <?php
                                        $id2 = $_REQUEST['Id'];
                                        $query2 = mysql_query("select * from table_servicereq where Id = '$id2'");
                                        $row2 = mysql_fetch_assoc($query2);
                                        ?>
                                        <h2 class="card-inside-title">Action Taken</h2>
                                        <div class="row clearfix">
                                            <div class="col-sm-6">
                                                <div class="form-group">
                                                    <div class="form-line">
                                                        <textarea name="at" cols="50" rows="10" class="form-control no-resize" placeholder="Please entrer the action taken..."><?php echo $row2['ActionTaken']; ?></textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <h2 class="card-inside-title">Date Finished</h2>
                                        <div class="row clearfix">
                                            <div class="col-sm-6">
                                                <div class="form-group">
                                                    <div class="form-line">
                                                        <input name="date" type="date" class="form-control" value="<?php echo $row2['DateFinished']; ?>"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
