...
if (!isset($_SESSION['INTRA_DATE_OB'])) {
    header("Location: OBList.php");
}

if (isset($_POST['updateLon'])) {

    $emp = $_REQUEST['emp'];
    $date = $_REQUEST['date'];
    
    $lonp = mysql_real_escape_string($_POST['lonp']);
    $lonw = $_POST['lonw'];
    $lonn = $_POST['lonname'];
    $lonc = count($_POST['lonw']);
    
    $lond = "";
    for ($i = 0; $i < $lonc; $i++) {
        if ($i == $lonc - 1) {
            $lond .= ($lonw[$i] != 0 ? trim($lonw[$i]) : "") . ' ~ ' . trim($lonn[$i]);
        } else {
            $lond .= ($lonw[$i] != 0 ? trim($lonw[$i]) : "") . ' ~ ' . trim($lonn[$i]) . ' + ';
        }
    }
    mysql_query("UPDATE table_otherbenefits SET Longevity_pay = '$lonp', Longevity_withtax = '$lond' WHERE Emp_no = '$emp' AND Longevity_date = '$date'");

    $_SESSION['INTRA_INDICATOR'] = 'true';
}
if (isset($_POST['updateHaz'])) {
    $emp = $_REQUEST['emp'];
    $date = $_REQUEST['date'];
    
    $hazp = mysql_real_escape_string($_POST['hazp']);
    $hazw = $_POST['hazw'];
    $hazn = $_POST['hazname'];
    $hazc = count($_POST['hazw']);
    
    $hazd = "";
    for ($i = 0; $i < $hazc; $i++) {
        if ($i == $hazc - 1){
            $hazd .= ($hazw[$i] != "" ? trim($hazw[$i]) : "") . ' ~ ' . trim($hazn[$i]);
        } else {
            $hazd .= ($hazw[$i] != "" ? trim($hazw[$i]) : "") . ' ~ ' . trim($hazn[$i]) . ' + ';
        }
    }
    
    mysql_query("UPDATE table_otherbenefits SET Hazard_pay = '$hazp', Hazard_withtax = '$hazd' WHERE Emp_no = '$emp' AND Hazard_date = '$date'");

    $_SESSION['INTRA_INDICATOR'] = 'true2';
}
if (isset($_POST['updateSublau'])) {
    $emp = $_REQUEST['emp'];
    $date = $_REQUEST['date'];
    
    $suba = mysql_real_escape_string($_POST['suba']);
    $laua = mysql_real_escape_string($_POST['laua']);
    $slwt = $_POST['sublauw'];
    $slnm = $_POST['sublauname'];
    $slco = count($_POST['sublauw']);
    
    $slde = "";
    for ($i = 0; $i < $slco; $i++) {
        if ($i == $slco - 1) {
            $slde .= ($slwt[$i] != "" ? trim($slwt[$i]) : "") . ' ~ ' . trim($slnm[$i]);
        } else {
            $slde .= ($slwt[$i] != "" ? trim($slwt[$i]) : "") . ' ~ ' . trim($slnm[$i]) . ' + ';
        }
    }
    mysql_query("UPDATE table_otherbenefits SET Subsistence_allowance = '$suba', Laundry_allowance = '$laua', Sublau_withtax = '$slde' WHERE Emp_no = '$emp' AND Sublau_date = '$date'");

    $_SESSION['INTRA_INDICATOR'] = 'true3';
}
?>
...
<html lang="en-US">
                        ...
                        <div class="card">
                            <div class="header">
                                <h2>
                                    ...
                                    <small>VERIFY OTHER BENEFITS</small>
                                </h2>
                            ...
                            <div style="background-color: #fff; min-height: 80vh;" class="body">
                                ...
                                ?>
                                <?php
                                if (isset($_REQUEST['cat']) && $_REQUEST['cat'] == 'lon') {
                                    ?>
                                <form action="EditOtherBenefits.php?emp=<?php echo $_GET['emp'] . '&date=' . $_GET['date'] . '&cat=' . $_GET['cat']; ?>" method="post">
                                        <table id="example" class="example table-bordered stripe row-border order-column" cellspacing="0" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>Name</th>
                                                    <th>Date Covered</th>
                                            ...
                                            <tbody>
                                                <?php
                                                $query = mysql_query("SELECT A.*, B.* FROM table_otherbenefits A"
                                                        . " LEFT JOIN table_empinfo B"
                                                        . "     ON A.Emp_no = B.Employee_Id WHERE Emp_no = '$emp' AND Longevity_date = '$datee'");
                                                while ($row = mysql_fetch_array($query)) {
                                                    ...
                                                    echo '<tr>';
                                                    echo '<td>' . $name . '</td>';
                                                    echo '<td>' . ($row['Date_covered'] == '0000-00-00' ? "" : date('M d, Y', strtotime($row['Date_covered']))) . '</td>';
                                                    echo '<td>' . ($row['Longevity_date'] == '0000-00-00' ? "" : date('M d, Y', strtotime($row['Longevity_date']))) . '</td>';
                                                    echo '<td><div style="margin-bottom: -5px; font-size: 10px;"><input class="font" type="text" name="lonp" class="form-control" value="' . ($row['Longevity_pay'] != '' ? str_replace(',', '', $row['Longevity_pay']) : 0) . '"></div></td>';
                                                    echo '<td>';
                                                    $londeduExp = explode('+', $row['Longevity_withtax']);
                                                    ...
                                                    for ($i = 0; $i < $londeduCount; $i++) {
                                                        ...
                                                        echo '<b>' . $londed1[1] . '</b> - ' . '<input type="hidden" name="lonname[]" value="' . $londed1[1] . '"><input class="font" type="text" name="lonw[]" class="form-control" value="' . $lonotherDeduc . '">' . '<br>';
                                                    }
                                                    echo '</td>';
                                                    echo '</tr>';
                                                }
                                        ...
                                        <input type="submit" name="updateLon" class="btn bg-teal btn-md waves-effect" value="Update">
                                    </form>
                                    <?php
                                        } elseif (isset($_REQUEST['cat']) && $_REQUEST['cat'] == 'haz') {
                                    ?>
                                    <form action="EditOtherBenefits.php?emp=<?php echo $_GET['emp'] . '&date=' . $_GET['date'] . '&cat=' . $_GET['cat']; ?>" method="post">
                                        <table id="example" class="example table-bordered stripe row-border order-column" cellspacing="0" width="100%">
                                            <thead>
                                            ...
                                            <tbody>
                                                ...
                                                <?php
                                                $query1 = mysql_query("SELECT A.*, B.* FROM table_otherbenefits A"
                                                        . " LEFT JOIN table_empinfo B"
                                                        . "     ON A.Emp_no = B.Employee_Id WHERE Emp_no = '$emp' AND Hazard_date = '$datee'");
                                                while ($row = mysql_fetch_array($query1)) {
                                                    ...
                                                    echo '<td>' . $name . '</td>';
                                                    echo '<td>' . ($row['Date_covered'] == '0000-00-00' ? "" : date('M d, Y', strtotime($row['Date_covered']))) . '</td>';
                                                    echo '<td>' . ($row['Hazard_date'] == '0000-00-00' ? "" : date('M d, Y', strtotime($row['Hazard_date']))) . '</td>';
                                                    echo '<td><div style="margin-bottom: -5px; font-size: 10px;"><input class="font" type="text" name="hazp" class="form-control" value="' . ($row['Hazard_pay'] != '' ? str_replace(',', '', $row['Hazard_pay']) : 0) . '"></td>';
                                                    echo '<td>'; //<div style="margin-bottom: -5px; font-size: 10px;">
                                                    $hazdeduExp = explode('+', $row['Hazard_withtax']);
                                                    ...
                                                    for ($i = 0; $i < $hazdeduCount; $i++) {
                                                        ...
                                                        echo '<b>' . $hazded1[1] . '</b> - ' . '<input type="hidden" name="hazname[]" value="' . $hazded1[1] . '"><input class="font" type="text" name="hazw[]" class="form-control" value="' . $hazotherDeduc . '">' . '<br>';
                                                    }
                                                    //echo '<input class="font" type="text" name="hazw" class="form-control" value="' . ($row['Hazard_withtax'] != '' ? str_replace(',', '', $row['Hazard_withtax']) : 0) . '">';
                                                    echo '</td>';
                                                    echo '</tr>';
                                                }
                                                ?>
                                        ...
                                        <input type="submit" name="updateHaz" class="btn bg-teal btn-md waves-effect" value="Update">
                                    </form>
                                    <?php
                                } elseif (isset($_REQUEST['cat']) && $_REQUEST['cat'] == 'sublau') {
                                    ?>
                                    ...
                                    <form action="EditOtherBenefits.php?emp=<?php echo $_GET['emp'] . '&date=' . $_GET['date'] . '&cat=' . $_GET['cat']; ?>" method="post">
                                        <table id="example" class="example table-bordered stripe row-border order-column" cellspacing="0" width="100%">
                                            <thead>
                                                <tr>
                                                    <th>Name</th>
                                                    <th>Date Covered</th>
                                                    <th>Subsistence and Laundry Date</th>
                                            ...
                                            <tbody>
                                                <?php
                                                $query2 = mysql_query("SELECT A.*, B.* FROM table_otherbenefits A"
                                                        . " LEFT JOIN table_empinfo B"
                                                        . "     ON A.Emp_no = B.Employee_Id Where Emp_no = '$emp' AND Sublau_date = '$datee'");
                                                while ($row = mysql_fetch_array($query2)) {
                                                    ...
                                                    echo '<td>' . $name . '</td>';
                                                    echo '<td>' . ($row['Date_covered'] == '0000-00-00' ? "" : date('M d, Y', strtotime($row['Date_covered']))) . '</td>';
                                                    echo '<td>' . ($row['Sublau_date'] == '0000-00-00' ? "" : date('M d, Y', strtotime($row['Sublau_date']))) . '</td>';
                                                    echo '<td><div style="margin-bottom: -5px; font-size: 10px;"><input class="font" type="text" name="suba" class="form-control" value="' . ($row['Subsistence_allowance'] != '' ? str_replace(',', '', $row['Subsistence_allowance']) : 0) . '"></td>';
                                                    echo '<td><div style="margin-bottom: -5px; font-size: 10px;"><input class="font" type="text" name="laua" class="form-control" value="' . ($row['Laundry_allowance'] != '' ? str_replace(',', '', $row['Laundry_allowance']) : 0) . '"></td>';
                                                    echo '<td>';
                                                    $sublaudeduExp = explode('+', $row['Sublau_withtax']);
                                                    ...
                                                    for ($i = 0; $i < $sublaudeduCount; $i++) {
                                                        ...
                                                        echo '<b>' . $sublauded1[1] . '</b> - ' . '<input type="hidden" name="sublauname[]" value="' . $sublauded1[1] . '"><input class="font" type="text" name="sublauw[]" class="form-control" value="' . $sublauotherDeduc . '">' . '<br>';
                                                    }
                                                    echo '</td>';
                                                    echo '</tr>';
                                                }
                                        ...
                                        <input type="submit" name="updateSublau" class="btn bg-teal btn-md waves-effect" value="Update">
                                    </form>
    ...
    if (isset($_SESSION['INTRA_INDICATOR']) && $_SESSION['INTRA_INDICATOR'] == 'true') {
        ...
        <script>
            $(document).ready(function () {
                swal("Successful!", "Longevity has successfully updated!", "success");
            })
        </script>
        <?php
    }
    ?> 
    <?php
    if (isset($_SESSION['INTRA_INDICATOR']) && $_SESSION['INTRA_INDICATOR'] == 'true2') {
        ?>
        <script>
            $(document).ready(function () {
                swal("Successful!", "Hazard has successfully updated!", "success");
            })
        </script>
        <?php
    }
    ?>
    <?php
    if (isset($_SESSION['INTRA_INDICATOR']) && $_SESSION['INTRA_INDICATOR'] == 'true3') {
        ?>
        <script>
            $(document).ready(function () {
                swal("Successful!", "Subsistence and Laundry has successfully updated!", "success");
            })
        </script>
        <?php
    }
            ...
            $('.example').on('click', 'a.confirmme', function (e) {
                ...
                function () {
                    if ($('#bene').val() === 'lon') {
                        $.ajax({
                            type: 'POST',
                            url: 'ViewOtherBenefits.php?emp=' + emp + '&date=' + date + '&act=app',
                            success: function () {
                                $('.example').attr("id");
                    ...
                    } else if ($('#bene').val() === 'haz') {
                        $.ajax({
                            type: 'POST',
                            url: 'ViewOtherBenefits.php?emp=' + emp + '&date=' + date + '&act=app1',
                            success: function () {
                                $('.example').attr("id");
                                a.closest('td').html('<i class="material-icons" style="color: green;">done</i>');
                                swal("Successful!", "The data is now verified.", "success");
                    ...
                    } else if ($('#bene').val() === 'sublau') {
                        $.ajax({
                            type: 'POST',
                            url: 'ViewOtherBenefits.php?emp=' + emp + '&date=' + date + '&act=app2',
                            success: function () {
                                $('.example').attr("id");
                                a.closest('td').html('<i class="material-icons" style="color: green;">done</i>');
                                swal("Successful!", "The data is now verified.", "success");
